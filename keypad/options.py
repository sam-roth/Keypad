...
UserConfigHome      = pathlib.Path(os.path.expanduser('~/.keypad'))
DefaultColorScheme  = colorscheme.SolarizedDark()
DefaultDriverMod    = 'keypad.qt.driver'
...
class GeneralSettings(Settings):
    ...
    driver_mod = Field(str, 'keypad.qt.driver')
    colorscheme = Field(Factory, 'keypad.core.colorscheme.SolarizedDark')
    user_config_home = Field(pathlib.Path, pathlib.Path(os.path.expanduser('~/.keypad')))
    ...
    selection = Field(Factory, 'keypad.buffers.selection.IndentingBacktabSelection')
